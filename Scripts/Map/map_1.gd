...
func setup_player_skills():
	...
	if player:
		...
		player.enable_swing = false
		player.enable_wall_jump = false
		
		# 3. Mở khóa kỹ năng lướt (Dash)
		player.enable_dash = true
		
		print("✅ Setup Map_1 thành công: Chỉ cho lướt và nhảy 1 nhịp!")
	else:
		print("❌ Lỗi cmnr: Không tìm thấy Player! Bồ check lại xem ném Player vào Map chưa nhé.")
